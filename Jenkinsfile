pipeline {
    ...
    stages {
        ...
        stage('Packaging/Pushing imagae') {
            ...
            steps {
                withDockerRegistry(credentialsId: 'dockerhub', url: 'https://index.docker.io/v1/') {
                    sh 'docker build -t jk911/springboot .'
                    sh 'docker push jk911/springboot'
                }
            }
        }
        ...
        stage('Deploy Spring Boot to DEV') {
            steps {
                echo 'Deploying and cleaning'
                sh 'docker image pull jk911/springboot'
                sh 'docker container stop jk911-springboot || echo "this container does not exist" '
                sh 'docker network create dev || echo "this network exists"'
                sh 'echo y | docker container prune '

                sh 'docker container run -d --rm --name jk911-springboot -p 8888:8888 --network dev jk911/springboot'
            }
        }
